...
#define X(name, type, num_bytes)                                               \
    type *name##FileRead(fileInfo_t *fileI) {                                  \
        ...
        if (buffer == NULL)                                                    \
            exit(EXIT_FAILURE);                                                \
        if (fread(buffer, num_bytes, fileI->size / num_bytes, fileI->file))    \
            return buffer;                                                     \
        else                                                                   \
            exit(EXIT_FAILURE);                                                \
    }                                                                          \
...
void fileDelete(const char *path) { unlink(path); }
...
void fileMove(const char *origin, const char *dest) { rename(origin, dest); }
...
fileInfo_t fileCopy(fileInfo_t fileI, char *dest) {
    ...
    if (test_file !=
        NULL) { // if a file with the smae name exist, now edits the name adding
                // a zero in the end, but in the future this will return an
                // fileInfo_t with NULL vars and with an ERROR signal.
        fclose(test_file);
    ...
    fileInfo_t new_file = {.file = fopen(new_path, "wb")};
