...
pathsList_t dirListRecursive(const char *path, u8 showHidden) {
    ...
    while (step < list.numItens) {
        ...
        if (dirExists(list.itensPaths[step])) {
            ...
            char ** Tp = (char **)realloc(list.itensPaths, sizeof(char *) * newTotal);
            if(Tp != NULL)
              list.itensPaths = Tp;
            for (size_t i = list.numItens, j = 0;
                 list.numItens < (i + temp.numItens); list.numItens++, j++) {
